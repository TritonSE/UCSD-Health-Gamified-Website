...
export default function BasicMaintenance() {
  const [activeCircle, setActiveCircle] = useState<number>();

  let activeCircleData = { header: "", text: "", width: 0, height: 0 };
  if (activeCircle) {
    activeCircleData = CIRCLE_DATA[activeCircle];
  }

  const handleCircleClick = (number: string) => {
    setActiveCircle(parseInt(number));
  };
  ...
  return (
    <section className={styles.container}>
      ...
      <BikeMaintSVG handleCircleClick={handleCircleClick} activeCircle={activeCircle} />
